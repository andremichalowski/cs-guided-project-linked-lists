...
def delete_node(node_to_delete):
    # Your code here
    '''
    Input: a reference to a node in a linked list
    that we want to delete from the linked list 
    Output: this function will mutate the linked list
    but it won't return anything
​
    One way we can delete a node from a linked list is 
    to change the node that refers to the node we want
    to delete so that it points to the node we want to
    delete's next node
​
    We need access to the node that refers to the node
    we want to delete.
​
    "So we're replacing the `node_to_delete`, and then
    deleting the next node"
​
    1. Changed the `node_to_delete`'s value to its 
       next node's value.
    2. Remove the `node_to_delete`'s next node by 
       having `node_to_delete`'s next refer to the 
       next node's next 
    '''
    next_node = node_to_delete.next
    
    # set the `node_to_delete`'s value to 
    # its next node's value 
    node_to_delete.value = next_node.value
​
    # set `node_to_delete`'s next to the next 
    # node's next 
